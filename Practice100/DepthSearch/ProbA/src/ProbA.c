...
int main (int argc, char *argv[]) {
    int i, j;
    volatile bool isallfound = false;
    ...
    _Graph G[MAX_N+1];
    // long long u[MAX_N+1], k[MAX_N+1], v[MAX_N+1][MAX_N+1];
    for (i=0; i<n; i++) {
        get1Number(MAX_N, 1, &G[i].u);
        get1Number(MAX_N, 1, &G[i].k);
        for (j=0; j<G[i].k; j++) {
            get1Number(MAX_N, 1, &G[i].v[j]);
        }
        G[i].nowv = 0;
        G[i].foundTime = 0;
        G[i].completeTime = 0;
    ...
    long long nowG = 0;
    long long beforeG;
    while (isallfound == false) {
        if (G[nowG].foundTime == 0) {
            G[nowG].foundTime = time;
            printf("nowG:%lld, foundTime:%d\n", nowG, time);
            time++;
        }
        if (G[nowG].nowv < G[nowG].k) {
            beforeG = nowG;
            nowG = G[nowG].v[G[nowG].nowv]-1;
            G[beforeG].nowv++;
            G[nowG].beforeG = beforeG;
            printf("nowG:%lld->%lld\n", beforeG, nowG);
        } else {
            if (G[nowG].completeTime == 0) {
                G[nowG].completeTime = time;
                printf("nowG:%lld, completeTime:%d\n", nowG, time);
                time++;
            }
            printf("nowG:%lld->%d, completeTime:%d\n", nowG, G[nowG].beforeG, G[nowG].completeTime);
            nowG = G[nowG].beforeG;
        }
        for (i=0; i<n; i++) {
            if (G[i].completeTime == 0) {
                isallfound = false;
                break;
            }
            isallfound = true;
    ...
    for (i=0; i<n; i++) {
        printf("%lld %d %d\n", G[i].u, G[i].foundTime, G[i].completeTime);
    }
    return 0;
}
