#include <stdio.h>
#include <ctype.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>

#define MAX_INT     19
#define MAX_N       30
#define MAX_AB      1000000000

#define DEBUG

int getDigit(long long unsigned int num);
long long unsigned int StrToInt(char* str, int num);
int getNumbers(int n, int max, int min, int *result);
int getCharacters(int max, char *result);
void print_err(char *ms);
int max(int a, int b);

void print_err(char *ms) {
#if defined DEBUG
    printf("ERROR  %s!!!\n", ms);
#endif
}

int getDigit(long long unsigned int num) {
    if (num == 0) {
        return 1;
    }
    
    int digit = 0;
    while (num > 0) {
        num /= 10;
        digit++;
    }
    return digit;
}


/* 標準入力から受け取った文字列を配列に入れて返す */
int getCharacters(int max, char *result) {
    int count = 0;
    char ch;

    while ((ch = getchar()) != '\n') {
        if (ch == ' ') {
            continue;
        }
        result[count] = ch;
        count++;
        if (count >= max) {
            print_err("too much S");
            return false;
        }
    }
    result[count] = '\0'; 
    return true;
}

/* 標準入力から空白を挟んでn個の整数を受け取り、配列に入れて返す */
int getNumbers(int n, int max, int min, int *result) {
    int count = 0;
    int num_count = 0;
    char ch;
    char buf[MAX_INT+1] = "";
    int max_digit = getDigit(max);

    while ((ch = getchar())) {
        if (ch == ' ' || ch == '\n') {
            if (buf[0] != '\0') {
                result[count] = StrToInt(buf, num_count);
                if (result[count] < min || result[count] > max) {
                    print_err("Num big or small");
                    return false;
                }
                count++;
                if (count > n) {       /* 標準入力に記載された整数がn個を超えたらエラー */
                    print_err("Num Count Over");
                    return false;
                }
                num_count = 0;
            }
            if (ch == ' ') {          
                continue;
            } else {
                break;
            }
        }

        buf[num_count] = ch;
        num_count++;
        if (num_count > max_digit) {   /* 数字の桁数がmax_intを超えたらエラー */
            print_err("Maxint Over");
            return false;
        }
    }

    if (count != n) {
        printf("count:%d, n:%d\n", count, n);
        print_err("count != n");
        return false;
    }
    return true;
}

long long unsigned int StrToInt(char* str, int num) {
    int i;
    long long unsigned int n = 0;
    for (i=0; i<num; i++) {
        if (isdigit(str[i])) {
            n = n * 10 + (str[i] - '0');
        } else {
            print_err("Input Number");
            break;
        }
    }
    return n;
}

int max(int a, int b) {
    if (a > b)
        return a;
    else
        return b;
}

int main (int argc, char *argv[]) {
    long long int Entry = MAX_AB;
    long long int Exit = 0;
    long long int i;
    long long int totalTime = 0;
    long long int totalTimeMin = 0;

    /* Nの取得 */
    int N;
    if (getNumbers(1, MAX_N, 1, &N) == false) {
        return 1;
    }

    /* A,Bの取得 */
    int AB[MAX_N][2];
    for (i=0; i<N; i++) {
        if (getNumbers(2, MAX_AB, 0, AB[i]) == false) {
            return 1;
        }
        if (AB[i][0] < Entry) {
            Entry = AB[i][0];
        }
        if (AB[i][1] > Exit) {
            Exit = AB[i][1];
        }
    }

    for (i=0; i<N; i++) {
        totalTimeMin += abs(AB[i][0] - Entry) + abs(AB[i][0] - AB[i][1]) + abs(AB[i][1] - Exit);
    }

    long long int difAB = 0;
    difAB = abs(Exit - Entry) / 2;
    long long int beforeEntry = Entry;
    long long int beforeExit = Exit;
    while (1) {
        if (difAB < 1) {
            difAB = 1;
        }

        // printf("Entry:%llu, Exit:%llu, time:%llu\n", Entry, Exit, totalTimeMin);
        totalTime = 0;
        Exit = beforeExit;
        Entry = beforeEntry + difAB;
        for (i=0; i<N; i++) {
            totalTime += abs(AB[i][0] - Entry) + abs(AB[i][0] - AB[i][1]) + abs(AB[i][1] - Exit);
        }
        if (totalTime <= totalTimeMin) {
            totalTimeMin = totalTime;
            beforeEntry = Entry;
            beforeExit = Exit;
            continue;
        }

        if (difAB > 1) {
            difAB /= 2;
            
            totalTime = 0;
            Entry = beforeEntry - difAB;    
            for (i=0; i<N; i++) {
                totalTime += abs(AB[i][0] - Entry) + abs(AB[i][0] - AB[i][1]) + abs(AB[i][1] - Exit);
            }
            totalTimeMin = totalTime;
            beforeEntry = Entry;
            beforeExit = Exit;
            continue;
        }
        Entry = beforeEntry;
        break;
    }

    difAB = abs(Exit - Entry) / 2;
    beforeEntry = Entry;
    beforeExit = Exit;
    while (1) {
        if (difAB < 1) {
            difAB = 1;
        }

        // printf("Entry:%llu, Exit:%llu, time:%llu\n", Entry, Exit, totalTimeMin);
        totalTime = 0;
        Entry = beforeEntry;
        Exit = beforeExit - difAB;
        for (i=0; i<N; i++) {
            totalTime += abs(AB[i][0] - Entry) + abs(AB[i][0] - AB[i][1]) + abs(AB[i][1] - Exit);
        }
        if (totalTime <= totalTimeMin) {
            totalTimeMin = totalTime;
            beforeEntry = Entry;
            beforeExit = Exit;
            continue;
        }
        
        Exit = beforeExit;
        if (difAB > 1) {        
            totalTime = 0;
            difAB /= 2;
            Exit = beforeExit + difAB;
            for (i=0; i<N; i++) {
                totalTime += abs(AB[i][0] - Entry) + abs(AB[i][0] - AB[i][1]) + abs(AB[i][1] - Exit);
            }
            totalTimeMin = totalTime;
            beforeEntry = Entry;
            beforeExit = Exit;
            continue;
        }
        Exit = beforeExit;
        break;

    }
    // printf("Entry:%llu, Exit:%llu\n", Entry, Exit);
    printf("%llu\n", totalTimeMin);
    return 0;
}
