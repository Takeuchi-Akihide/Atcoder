...
int main (int argc, char *argv[]) {
    ...
    /* wの取得 */
    long long RC[MAX_N][2];
    for (i=0; i<N; i++) {
        getNumbers(2, MAX_N, 1, RC[i]);
    }
    ...
    for (i=0; i<MAX_N; i++) {
        for (j=i+1; j<MAX_N; j++) {
            if (j - i == 1) {
                dp[i][j] = RC[i][0] * RC[i][1] * RC[j][1];
            } else {
                dp[i][j] = MAX_N * MAX_N * MAX_N;
            }
        }
    }
    int l, r, m;
    for (i=1; i<=N; i++) {
        for (l=0; l<N; l++) {
            ...
            if (r > N) continue;
            for (m=l; m<r; m++) {
                dp[l][r] = min(dp[l][r], dp[l][m] + dp[m+1][r] + RC[l][0] * RC[m][1] * RC[r][1]);
            }
        }
    }
    // for (i=0; i<=N; i++) {
