...
/* array[i-1]>=x>array[i] のとき、iを返す */
/* arrayは降順に並べ替えておく */
long long binarySearchNumR(long long x, long long *array, long long n) {
    long long min = 0;
    long long max = n;
    long long mid;
    if (x > array[0]) {
...
int main (int argc, char *argv[]) {
    ...
    long long pattern[MAX_N];
    pattern[0] = A[0];
    int color = 1;

    for (i=1; i<N; i++) {
        int num = binarySearchNumR(A[i], pattern, color);
        if (num == color) {
            color++;
        }
        pattern[num] = A[i];
