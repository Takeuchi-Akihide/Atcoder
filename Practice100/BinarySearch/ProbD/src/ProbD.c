...
bool pointUnderX(long long x, long long HS[][2], long long N){
    ...
    for (i=0; i<N; i++) {
        ...
        times[i] = (x - HS[i][0]) / HS[i][1];
        // printf("x:%lld H:%lld S:%lld times[%lld]:%lld\n", x, HS[i][0], HS[i][1], i, times[i]);
    }
    qsort(times, N, sizeof(long long), cmpnum);
    for (i=0; i<N; i++) {
        if (times[i] < i){
            return false;
        }
...
int main (int argc, char *argv[]) {
    ...
    /* H,Sの取得 */
    long long HS[MAX_N][2];
    for (i=0; i<N; i++) {
        if (getNumbers(2, MAX_H, 1, HS[i]) == false) return 1;
    }

    long long max_HS[MAX_N];
    for (i=0; i<N; i++) {
        max_HS[i] = HS[i][0] + HS[i][1] * (N-1);
    }
    qsort(max_HS, N, sizeof(long long), cmpnum);

    long long min = 0;
    long long max = max_HS[N-1] * 2;
    long long mid;
    while (min <= max) {
        mid = (min + max) / 2;
        if (max == min) {
            // if (pointUnderX(min, HS, N) == true) {
                // printf("%lld\n", min);
            // } else {
            //     printf("%lld\n", max);
            // }
            break;
        } else if (pointUnderX(mid, HS, N) == true) {
            max = mid;
        } else {
            min = mid + 1;
        }
