...
int main (int argc, char *argv[]) {
    ...
    long long Q = Input1[1];

    /* A,Bの取得 */
    long long A[MAX_N], C[MAX_N];
    getNumbers(N, MAX_A, 1, A);
    getNumbers(N, MAX_A, 1, C);
    long long d[MAX_N];     // d[i]:=町i→町i+1 の距離
    for (i=0; i<N-1; i++) {
        d[i+1] = modpow(A[i], A[i+1]);
    }
    long long d2[MAX_N];    // d2[i]:=町1→町i の距離
    d2[1] = 0;
    for (i=2; i<=N; i++) {
        d2[i] = (d[i-1] + d2[i-1]) % MOD;
    }

    long long res = d2[C[0]];
    for (i=1; i<Q; i++) {
        long long tmp = 0;
        if (C[i] > C[i-1]) {
            tmp = (d2[C[i]] - d2[C[i-1]]) % MOD;
        } else {
            tmp = (d2[C[i-1]] - d2[C[i]]) % MOD;
        }
        if (tmp < 0) {
            tmp += MOD;
        }
