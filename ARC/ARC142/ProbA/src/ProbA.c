...
#define MAX_INT     19
// long long MAX_N = (long long)pow(10, 12);
...
long long reverseK(long long K) {
    long long res = 0;
    while (K > 0) {
        res = res * 10 + K % 10;
        K /= 10;
...
int main (int argc, char *argv[]) {
    ...
    /* Nの取得 */
    long long NK[2];
    getNumbers(2, (long long)pow(10, 12), 1, NK);

    long long N = NK[0];
    long long K = NK[1];
    long long K_r = reverseK(K);
