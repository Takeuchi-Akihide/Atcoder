...
int cmpnum(const void * n1, const void * n2)
{
	if (((_data *)n1)->left > ((_data *)n2)->left)
	{
		return 1;
	}
	else if (((_data *)n1)->left < ((_data *)n2)->left)
	{
		return -1;
	}
	else
...
int main (int argc, char *argv[]) {
    ...
    long long LR[2];
    for (i=0; i<N; i++) {
        getNumbers(2, MAX_N, 1, LR);
        LRdata[i].left = LR[0];
        LRdata[i].right = LR[1];
    }
    qsort(LRdata, N, sizeof(_data), cmpnum);

    _data shugo[MAX_N];
    int shugoNum = 0;
    int count = 0;
    shugo[0].left = LRdata[0].left;
    shugo[0].right = LRdata[0].right;

    for (i=1; i<N; i++) {
        if (LRdata[i].left <= shugo[count].right) {
            if (LRdata[i].right >= shugo[count].right) {
                shugo[count].right = LRdata[i].right;
            }
        } else {
            count++;
            shugo[count].left = LRdata[i].left;
            shugo[count].right = LRdata[i].right;
        }
    }
    for (i=0; i<=count; i++) {
        printf("%lld %lld\n", shugo[i].left, shugo[i].right);
    }
    return 0;
}
