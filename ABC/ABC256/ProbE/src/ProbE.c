...
int main (int argc, char *argv[]) {
    ...
    /* A,Bの取得 */
    long long X[MAX_N], C[MAX_N];
    getNumbers(N, MAX_N, 1, X);
    getNumbers(N, MAX_C, 1, C);
    _data Person[MAX_N];

    for (i=1; i<=N; i++) {
        Person[i].hateX = X[i-1];
        Person[i].C = C[i-1];
        Person[i].isChecked = false;
    }
    ...
    for (i=1; i<=N; i++) {
        ...
        // Person[i].isChecked = true;
        bool isCheckedNow[MAX_N] = {0};
        nowi = i;
        bool isroop = false;
        bool isPlus = false;
        long long roopstart;
        while(1) {
            if (isroop) {
                Person[nowi].isChecked = true;
                if (min > Person[nowi].C) {
                    min = Person[nowi].C;
                }
                if (roopstart == nowi) {
                    isPlus = true;
                ...
                if (Person[nowi].isChecked == true) {
                    break;
                }
                if (isCheckedNow[nowi] == true) {
                    isroop = true;
                    roopstart = nowi;
                    min = Person[nowi].C;
                }
                isCheckedNow[nowi] = true;
            }
            nowi = Person[nowi].hateX;
        }
        if (isPlus) {
            sum += min;
        }
