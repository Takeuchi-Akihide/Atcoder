...
int main (int argc, char *argv[]) {
    ...
    /* Nの取得 */
    long long Input1[2];
    getNumbers(2, MAX_X, 1, Input1);

    long long N = Input1[0];
    long long X = Input1[1];

    /* A,Bの取得 */
    long long A[MAX_N], B[MAX_N];
    for (i=0; i<N; i++) {
        get1Number(MAX_X, 1, &A[i]);
        get1Number(MAX_X, 1, &B[i]);
    }

    long long min;
    long long play1[MAX_N];
    play1[0] = A[0] + B[0];
    min = play1[0] + B[0] * (X - 1);
    for (i=1; i<N; i++) {
        if (X - i - 1 < 0) {
        ...
        play1[i] = play1[i-1] + A[i] + B[i];
        long long tmp = play1[i] + B[i] * (X - i - 1);
        if (tmp <= min) {
            min = tmp;
        }
    }
